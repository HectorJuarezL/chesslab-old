...
class MCTSNode:
    ...
    def fill_nodes(self,node,depth):
        (game_state, parent, num_rollouts, win_counts) #tablas por nivel de profundidad
        moves = list(node.game_state.legal_moves)
        np_moves = np.zeros((len(moves),8,8,3))
        for m in moves:
