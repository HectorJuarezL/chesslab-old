...
class MCTSNode:
    def __init__(self, game_state, parent = None, move = None, value = [0.0,0.0], bot = None, is_root = False):
        self.win_counts = np.zeros([2,],dtype=np.float32)
        self.value=np.zeros([2,],dtype=np.float32)
        self.game_state = game_state
        self.parent = parent
        self.move = move
        self.num_rollouts = 1 if not is_root else 0
        self.children = []
        ...
        if self.is_terminal():
            ...
            self.num_rollouts += len(self.unvisited_moves)
            for i in range(self.num_rollouts-1): #se le resta 1 para obtener len(self.unvisited_moves) 
                self.add_child()
    ...
    def add_child(self):
        ...
        new_value = self.unvisited_values.pop()
        new_game_state = self.game_state.copy(stack=False) #crea una copia del estado de juego
        new_game_state.push(new_move) #realiza el movimiento seleccionado
        new_node = MCTSNode(game_state=new_game_state, parent=self, move=new_move, value = new_value) #crea un nuevo nodo
        self.value+=new_node.value
        self.children.append(new_node) #añade el nodo a su lista de hijos
        return new_node #retorna el nuevo nodo

    def record_win(self, result):
        self.win_counts += result[0]
        self.num_rollouts += result[1]


    def result_simulation(self):
        return self.value,self.num_rollouts
